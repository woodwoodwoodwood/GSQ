#!/usr/bin/env python3
"""
Generic serving benchmark (vLLM/OpenAI style).

Features (similar style to user's script):
- request_rate + concurrency based load generation
- async/sync modes
...
Examples:
  python scripts/benchmark_throughput.py \
    ...
    --backend openai-chat \
    --dataset_path benchmark/benchmark_input.csv \
    --model Qwen3-30B-A3B-Instruct-2507 \
    --stream --concurrency 32 --request_rate inf \
    --max_new_tokens 128 --perf_csv /tmp/perf.csv
"""
...
def percentile(values: List[float], p: int) -> float:
    ...
    # same spirit as np.percentile default linear interpolation
    rank = (len(values_sorted) - 1) * (p / 100.0)
    low = int(rank)
    high = min(low + 1, len(values_sorted) - 1)
    w = rank - low
    return values_sorted[low] * (1 - w) + values_sorted[high] * w
...
def args_config() -> argparse.Namespace:
    ...
    parser.add_argument("--request_rate", type=float, default=float("inf"),
                        help="requests per second, inf means fire as fast as possible")
    ...
    parser.add_argument("--percentiles", nargs="+", type=int, default=[99], help="e.g. 50 90 99")
    ...
    parser.add_argument("--backend", type=str, default="openai-chat", choices=["openai-chat", "vllm"])

    parser.add_argument("--prompt_num", type=int, default=0)
    parser.add_argument("--model", type=str, default="default", help="model id/path for server request")
...
def adjust_list_length(inputs: List[str], args: argparse.Namespace) -> List[str]:
    ...
    if args.prompt_num > len(inputs):
        ...
        if len(inputs) * repeat_times != args.prompt_num:
            raise ValueError(f"prompt_num={args.prompt_num} 不能整倍数扩展当前数据长度={len(inputs)}")
...
def _extract_stream_delta(output: dict, args: argparse.Namespace) -> Tuple[str, int, int]:
...
def _extract_nonstream_text(output: dict, args: argparse.Namespace) -> Tuple[str, int, int]:
    if args.backend == "openai-chat":
        choice = (output.get("choices") or [{}])[0]
            ...
            try:
                async with session.post(api_url, headers=headers, data=req_data) as response:
                    ...
                    if args.stream:
                        ...
                        async for chunk_bytes, _ in response.content.iter_chunks():
                            ...
                            chunk_acc += chunk
                            # split by lines because SSE may contain multiple data lines
                            ...
                            for line in lines[:-1]:
                                ...
                                if obj.get("__done__"):
                                    continue
                                output = obj
                                delta_text, p_tok, c_tok = _extract_stream_delta(obj, args)
                                if delta_text:
                        ...
                        if chunk_acc.strip():
                            ...
                            if obj and not obj.get("__done__"):
                                output = obj
                                _, p_tok, c_tok = _extract_stream_delta(obj, args)
                                if p_tok:
                                    prompt_tokens = p_tok
                                if c_tok:
                                    completion_tokens = c_tok
                        ...
                        server_text, prompt_tokens, completion_tokens = _extract_nonstream_text(output, args)
                        # non-stream fallback: treat first token as full response completion
                ...
                if output is not None and "error" not in output:
                    if completion_tokens <= 0:
                        # fallback approximation when server does not return usage
